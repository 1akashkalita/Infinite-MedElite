...
| TBD | render (SnapshotApp wiring + web preview) | 2 | CLM-01 / CLM-03 / SC#1 | — | hospMetrics captured from `json.hospMetrics`, cleared on error, threaded as the 4th `assembleViewModel` arg; 12 labels render after "Quality of Resident Care" | manual + tsc | `npx tsc --noEmit` (type/arg correctness) **+ blocking human checkpoint (Plan 04 Task 3)** — see Manual-Only below | n/a | ⬜ pending |
| TBD | pdf | 2 | CLM-03 / SC#4 | — | PDF buffer for 686123 contains "Short Term Hospitalization" **and a garbled label** (e.g. bare "ED Visit") + the 12 rows | integration | `npx vitest run tests/api/export-pdf.test.ts` (latin1 includes both labels) | ✅ (extend) | ⬜ pending |
...
| SnapshotApp captures `json.hospMetrics`, clears it on error, and threads it as the 4th `assembleViewModel` arg | CLM-01 / Plan 04 T1 | Client React state/effect wiring needs a DOM; this project's Vitest is **node env with no jsdom/RTL** and the phase adds **no new packages** — an RTL component test is out of scope here | tsc proves the 4th-arg type + key type; the response *shape* (`body.hospMetrics`) is asserted in `facility.test.ts`; the render-with-metrics path is asserted in `export-pdf.test.ts`. The residual risk (wrong response key, missing clear-on-error) is closed by the **blocking human checkpoint (Plan 04 Task 3)**: looking up 686123 shows all 12 rows — a wrong key would surface the degraded line instead |
